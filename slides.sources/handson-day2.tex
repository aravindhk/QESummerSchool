...
%
Saddle points on the {\em Potential Energy Surface (PES)}, which correspond to {\em Transition Sates (TS)} of chemical reactions, can be found by means of the {\em Nudged Elastic Band (NEB)} method. 
...
so-called first- and last-image, corresponding to reactants and
products.
...
The resulting plot is should look something like this. 
\begin{figure}
  \centering
...
the third NEB image, which is not a good guess. If we use
\card{INTERMEDIATE\_IMAGE} they can be placed closer together, which
is a lot closer to the actual transition state and the NEB calculation
does not require as many steps in order to converge. This is shown
schematically below: 
...
the calculation itself will also divided into two parts, in order to
achieve convergence more quickly.

We will be using climbing-image NEB (CI-NEB), however initially we
cannot say which image is the {\em climbing-image} (the one with the
highest energy), because its ID can change from iteration to iteration,
depending on how far from convergence we are.

Thus we will first perform a plain NEB (\var{CI\_scheme = 'no-CI'}) in
order to stabilize the pathway and only once its stabilized we will
switch on the CI-NEB with \var{CI\_scheme~=~'auto'}. 

{\bf{Steps:}}
\begin{itemize}
\item{Read \file{neb.H2-diss.Al100-2x1-2L.in} and try to understand it. This file will serve as the main input for the \prog{pwtk} script. Visualize the initial guess by :\\
    \exec{xcrysden --pwi neb.H2-diss.Al100-2x1-2L.in}}
\item{Read the \file{neb.pwtk} script and try to understand it. }
\item{Run the example by executing:\\ \exec{pwtk neb.pwtk >\& neb.log \& }}
\item{Once the calculation converges analyze \file{neb.noCI.out} and \file{neb.noCI.out} and check the number of steps and the reaction energy barrier. The reaction energy graph can be plotted by: \\
    \exec{gnuplot H2-diss.Al100-2x1-2L.gp} \\
    and the corresponding path visulized by: \\
    \exec{xcrysden --axsf H2-diss.Al100-2x1-2L.axsf } }
\end{itemize}
\begin{figure}
  \centering
    \includegraphics[width=13.0cm]{figs/NEB-H2onAl100-results.pdf}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\head{5.1 PWTK: EOS utility} In this example the {\em
  equation-of-state (EOS)} will be calculated using \prog{pwtk}. Run
this calculation by typing:

\exec{pwtk eos.Rh-bulk.pwtk > eos.Rh-bulk.log \& }

This procedure performs 15 scf calculations with different different
lattice parameters (using the initial guess as input) and then uses
the results as input for \prog{ev.x}, to calculate EOS in accordance
with the four schemes. The results are then summarized in
\file{eos.Rh-bulk.RESULTS}.

\begin{figure}
\centering
\includegraphics[width=16.0cm]{figs/LATTICE_PARAMETERS.png}
\end{figure}
